...
export default class Danmuku {
    ...
    makeWait(danmu) {
        ...
        if (danmu.$ref) {
            ...
            danmu.$ref.style.setProperty('--slide-start', `${0}px`);
            danmu.$ref.style.animationPlayState = 'running';

            danmu.$ref.style.animation = `slideIn linear 0s forwards`;
                        ...
                        if (danmu.$ref) {
                            if (!this.isStop && top !== undefined) {
                                ...
                                case 0: {
                                    ...
                                    danmu.$ref.style.setProperty('--slide-start', `${distance}px`);
                                    danmu.$ref.style.animationPlayState = 'running';
                                    danmu.$ref.style.animation = `slideIn ${danmu.$restTime}s linear forwards`;
                                    break;
                                }
                                case 1:
                                    // falls through
    ...
    resize() {
        ...
        this.filter('emit', (danmu) => {
            ...
            case 0: {
                ...
                // danmu.$ref.style.animation = `slideIn 10s linear forwards`;
                break;
            }
            default:
                break;
    ...
    continue() {
        ...
        this.filter('stop', (danmu) => {
            ...
            case 0: {
                ...
                // danmu.$ref.style.transition = `transform ${danmu.$restTime}s linear 0s`;
                // danmu.$ref.style.setProperty('--slide-start', `${-distance}px`);
                // danmu.$ref.style.animation = `slideIn ${danmu.$restTime}s linear 0s forwards`;
                danmu.$ref.style.animationPlayState = 'running';
                break;
            }
            default:
    ...
    suspend() {
        ...
        this.filter('emit', (danmu) => {
            ...
            case 0: {
                ...
                // danmu.$ref.style.transition = 'transform 0s linear 0s';
                // danmu.$ref.style.setProperty('--slide-start', `${translateX}px`);
                // danmu.$ref.style.animation = `slideIn 10s linear forwards`;
                danmu.$ref.style.animationPlayState='paused';
                // danmu.$ref.style.animationPlayState = 'running';
